    ...
    switch (op) {
        ...
        case OVAL_SET_OPERATION_INTERSECTION:
            if (f1 == SYSCHAR_FLAG_ERROR) {
                ...
                } else if (f2 == SYSCHAR_FLAG_NOT_COLLECTED || f2 == SYSCHAR_FLAG_COMPLETE ||
                           f2 == SYSCHAR_FLAG_INCOMPLETE) {
                    result = SYSCHAR_FLAG_INCOMPLETE;
                }
            } else if (f1 == SYSCHAR_FLAG_DOES_NOT_EXIST) {
                result = SYSCHAR_FLAG_DOES_NOT_EXIST;
                ...
                if (f2 == SYSCHAR_FLAG_ERROR) {
                    ...
                    result = SYSCHAR_FLAG_DOES_NOT_EXIST;
                } else if (f2 == SYSCHAR_FLAG_COMPLETE || f2 == SYSCHAR_FLAG_INCOMPLETE ||
                           f2 == SYSCHAR_FLAG_NOT_APPLICABLE || f2 == SYSCHAR_FLAG_NOT_COLLECTED) {
                    result = SYSCHAR_FLAG_NOT_COLLECTED;
                }
            } else if (f1 == SYSCHAR_FLAG_NOT_APPLICABLE) {
                if (f2 == SYSCHAR_FLAG_ERROR) {
            ...
            if (f1 == SYSCHAR_FLAG_ERROR) {
                ...
                if (f2 == SYSCHAR_FLAG_ERROR) {
                    result = SYSCHAR_FLAG_ERROR;
                           ...
                           f2 == SYSCHAR_FLAG_NOT_APPLICABLE) {
                    result = SYSCHAR_FLAG_COMPLETE;
                ...
                if (f2 == SYSCHAR_FLAG_ERROR) {
                    result = SYSCHAR_FLAG_ERROR;
                           ...
                           f2 == SYSCHAR_FLAG_DOES_NOT_EXIST) {
                    result = SYSCHAR_FLAG_INCOMPLETE;
...
SEXP_t *probe_item_create(oval_subtype_t item_subtype, probe_elmatr_t *item_attributes[],
    ...
    while (value_name != NULL) {
        ...
        if (probe_item_add_value(item, value_name, value_type, pvalue)) {
            SEXP_free(item);
            va_end(ap);
            return NULL;
        }
...
int probe_create_external_probe_query(SEXP_t *in, oval_external_probe_item_t** out_ext_query) {
    int ret = 0;
...
int probe_collect_external_probe_items(probe_ctx *ctx, oval_subtype_t type, oval_syschar_status_t status, oval_external_probe_item_list_t *ext_items) {
    int ret = 0;
