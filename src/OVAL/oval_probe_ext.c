 ...
 */
#ifdef OVAL_EXTERNAL_PROBES_ENABLED
oval_pext_t *oval_pext_new(void *sess_ptr, struct oval_syschar_model **model, void *pdata) {
    oval_pext_t *pext;

    pext = (oval_pext_t*)calloc(1, sizeof(oval_pext_t));
    if(pext == NULL) {
        goto fail;
    }
    ...
    probe_executor_ctx_t pexec_ctx = {
        ...
        .probe_cmd_handlers = {
                .obj_eval = oval_probe_cmd_obj_eval,
...
{
    if(pext != NULL) {
        probe_executor_free(pext->pexec);
    }
    free(pext);
}
#else
...
int oval_pext_reset(oval_pext_t *pext) {
    return probe_executor_reset(pext->pexec);
}
#endif
...
int oval_probe_sys_handler_exec(oval_pext_t *pext, oval_subtype_t type, struct oval_sysinfo **out) {
    int ret;
    SEXP_t *query, *obj = NULL;

    __attribute__nonnull__(pext->pexec);
    ...
    ret = probe_executor_exec(pext->pexec, &req);
    if(ret != 0) {
        goto fail;
    }
    ret = oval_probe_sexp_to_sysinfo(obj, *pext->model, out);
...
int oval_probe_ext_handler_exec(oval_pext_t *pext, oval_subtype_t type, struct oval_syschar *syschar) {
    int ret;
    struct oval_object *object;
    ...
    object = oval_syschar_get_object(syschar);
    ret = oval_object_to_sexp(pext->sess_ptr, oval_subtype_to_str(oval_object_get_subtype(object)), syschar, &query);
    if(ret != 0) {
        goto fail;
    }
    probe_request_t req = {
