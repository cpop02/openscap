...
static void dump_oval_external_probe_item(const char *prefix, oval_external_probe_item_t *fields)
{
    if(fields == NULL)
        return;
    ...
    OVAL_EXTERNAL_PROBE_ITEM_FOREACH(fields, name, value, {
        oval_datatype_t type = oval_external_probe_item_value_get_datatype(value);

        char val_str[16384];
        switch (type) {
            case OVAL_DATATYPE_STRING:
                snprintf(val_str, 16384, "%s", oval_external_probe_item_value_get_string(value));
                val_str[16383] = 0;
                break;
            case OVAL_DATATYPE_BOOLEAN:
                sprintf(val_str, "%s", oval_external_probe_item_value_get_boolean(value) ? "true" : "false");
                break;
            case OVAL_DATATYPE_INTEGER:
                sprintf(val_str, "%lld", oval_external_probe_item_value_get_integer(value));
                break;
            case OVAL_DATATYPE_FLOAT:
                sprintf(val_str, "%lf", oval_external_probe_item_value_get_float(value));
                break;
            default:
...
static void
datapoint_to_xml(xmlNodePtr parent, const char *probe_type, const char *id, oval_external_probe_item_t *fields)
{
    if (fields == NULL)
        return;
    ...
    OVAL_EXTERNAL_PROBE_ITEM_FOREACH(fields, name, value, {
        ...
        switch (type) {
            ...
            case OVAL_DATATYPE_INTEGER:
                ...
                break;
            case OVAL_DATATYPE_FLOAT:
                sprintf(val_str, "%f", oval_external_probe_item_value_get_float(value));
                break;
            default:
                sprintf(val_str, "UNKNOWN");
        }
...
default_external_probe(void *ctx, oval_subtype_t probe_type, char *id, oval_external_probe_item_t *values)
{
    const char *probe_type_str;
    oval_external_probe_item_list_t *items = NULL;
    xmlNodePtr datapointsNode = (xmlNodePtr) ctx;

    probe_type_str = oval_subtype_get_text(probe_type);
    printf("EXTPROBE: default_external_probe(%p, %s, %s)\n", ctx, probe_type_str, id);
    dump_oval_external_probe_item("EXTPROBE: default_external_probe", values);

    oval_external_probe_result_t *res = oval_external_probe_result_new(id);
    oval_external_probe_item_t *vars = NULL;
    ...
    if (probe_type == OVAL_INDEPENDENT_FAMILY) {
        vars = oval_external_probe_item_new(
                "family", oval_external_probe_item_value_new_string("windows"),
                NULL);
    } else if (probe_type == OVAL_INDEPENDENT_SYSCHAR_SUBTYPE) {
        vars = oval_external_probe_item_new(
    ...
    if (vars == NULL) {
        ...
        if (items == NULL) {
            printf("EXTPROBE: default_external_probe(%p, %s, %s) could not create item list\n", ctx, probe_type_str, id);
            status = SYSCHAR_STATUS_ERROR;
        }
    }
...
static char* external_family_probe(void *ctx)
{
    printf("EXTPROBE: external_family_probe(%p)\n", ctx);
    return strdup("windows");
}

static oval_external_probe_item_t* external_environment_probe(void *ctx)
{
    printf("EXTPROBE: external_environmentvariable_probe(%p)\n", ctx);
...
static oval_external_probe_item_t* external_system_info_probe(void *ctx)
{
    printf("EXTPROBE: external_system_info_probe(%p)\n", ctx);
    return oval_external_probe_item_new(
            "os_name", oval_external_probe_item_value_new_string("Windows"),
            "os_version", oval_external_probe_item_value_new_string("XP"),
            "os_architecture", oval_external_probe_item_value_new_string("x86"),
...
static void fill_external_probe_eval_funcs(oval_external_probe_eval_funcs_t *eval)
{
    ...
    eval->family_probe = external_family_probe;
    eval->system_info_probe = external_system_info_probe;
    eval->environment_probe = external_environment_probe;
...
#ifdef EXTERNAL_PROBE_COLLECT
    xmlDocPtr datapointsDoc = NULL;
    xmlNodePtr datapointsNode = NULL;
    if (action->f_datapoints != NULL) {
        datapointsDoc = xmlNewDoc(BAD_CAST "1.0");
        if (datapointsDoc == NULL) {
            // oscap_setxmlerr(xmlGetLastError());
            oscap_print_error();
            return ret;
        }
        datapointsNode = xmlNewNode(NULL, BAD_CAST "datapoints");
        xmlDocSetRootElement(datapointsDoc, datapointsNode);
    }

    oval_external_probe_eval_funcs_t eval;
    fill_external_probe_eval_funcs(&eval);
    eval.probe_ctx = datapointsNode;
...
bool getopt_oval_eval(int argc, char **argv, struct oscap_action *action)
{
    ...
    while ((c = getopt_long(argc, argv, "o:", long_options, NULL)) != -1) {
        switch (c) {
            case OVAL_OPT_RESULT_FILE:
                action->f_results = optarg;
                break;
            case OVAL_OPT_REPORT_FILE:
                action->f_report = optarg;
                break;
            case OVAL_OPT_DATAPOINTS_FILE:
                action->f_datapoints = optarg;
                break;
            case OVAL_OPT_ID:
                action->id = optarg;
                break;
            case OVAL_OPT_VARIABLES:
                action->f_variables = optarg;
                break;
            case OVAL_OPT_DIRECTIVES:
                action->f_directives = optarg;
                break;
            case OVAL_OPT_DATASTREAM_ID:
                action->f_datastream_id = optarg;
                break;
            case OVAL_OPT_OVAL_ID:
                action->f_oval_id = optarg;
                break;
            case 0:
                break;
            default:
    ...
    while ((c = getopt_long(argc, argv, "o:", long_options, NULL)) != -1) {
        switch (c) {
            ...
            case OVAL_OPT_VARIABLES:
                action->f_variables = optarg;
                break;
            case OVAL_OPT_SYSCHAR:
                action->f_syschar = optarg;
                break;
            case 0:
                break;
            default:
